...
Následuje ukázka pseudokódu jak by vypadala procedura Cohesion. Kód je v tomto pseudokódu obdobný jako kód zarovnání výjma řádku s normalizováním sumárního vektoru, avšak v implementační části je tento kód složitější.
...
Výsledná aplikace je naprogramována v jazyce C++ a jako hlavní nástroj jsem použil Visual Studio verze 2017 \cite{linkToVisualStudio}, protože je nejpoužívanějším IDE\footnote{Integrated Development Environment - program, který usnadňuje práci programátorům \cite{linkToIDE})} pro vývoj \cite{linkToTopIDE}. 
...
\subsection{Modulární architektura}
%\begin{figure}[H]\centering\includegraphics[width=0.62\textwidth]{Figures/screen3.png}
%	\caption{Ukázka konfigurace knihoven pomocí systémových proměnných}
%\end{figure}
