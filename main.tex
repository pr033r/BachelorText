...
\begin{figure}[H]\centering\includegraphics[width=0.5\textwidth]{Figures/alignment.jpg}{zarovnani}
...
Vůči 100 agentům ve scéně měla starší verze aplikace využití 110 MB včetně použitých textur a objektů, po těchto doposud zmíněných optimalizací bylo využití 91 MB.
\\\\
...
Všechna pravidla jsou aplikována jako součet výsledků jednotlivých pravidel \textit{(viz kapitola \ref{sec:aplikovani-tri-pravidel})}. \\V této práci k sečtení a vytvoření výsledného vektoru slouží metoda \textit{appplyForce(...))}, která přidá do výsledného vektoru \textit{acceleratoin} další vektor \textit{(výsledek jednoho z pravidel)}. Výsledná pozice agenta je pak ovlivněna tímto vektorem.

Ukázku můžeme vidět níže.

\begin{lstlisting}[label=src:seek,caption=Nastavení výsledné pozice agenta] 
void Boids::update() {
...
Po vynásobení konstantou 0.4, která slouží pro nastavení citlivosti \textit{(může a nemusí být použita, zde je nastavena po experimentech jako ideální)} následuje přidání a limitování vektoru a vypočtení výsledné pozice. Na konci musíme vektor \textit{acceleration} vynulovat pro použití v další iteraci.
...
\begin{lstlisting}[label=src:mapSave,caption=Způsob uložení mapy v paměti]
...
\begin{lstlisting}[label=src:mapSave,caption=Princip metody getArriveVector(...)]
...
První část testování je zaměřena na rychlost a plynulost aplikace. Jaké byly hodnoty FPS při daném počtu agentů, počtu pater a složitosti budovy. Hodnoty první a druhé tabulky odpovídají času před zapnutím simulace a času po simulaci.



\begin{table}[H]
	\centering
	\caption{Tabulka hodnot FPS úniku z budovy před začátkem simulace}
	\label{tab:ramoptimalization}
